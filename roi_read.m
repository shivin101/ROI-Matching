...
for i=1:size(img,2)
    ...
    views(i)=struct('desc',feature_arr{i},'img',max(img{i},[],3)/5000,'frame', ...
...
[jMatch,jmInfo] = runJointMatch(pMatch,C,'Method','pg','univsize',30, ...
    'rank',3,'lambda', 0.1);
    ...
    for i = 1:size(pMatch,1)
        for j = i+1:size(pMatch,2)
            ...
            if ~isempty(pMatch(i,j).X)
                fig = subplot('position',[0 0.5 1 0.48]);
...
roiDict = cell(size(pMatch,1),size(pMatch,2))
 for i = 1:size(pMatch,1)
        for j = i+1:size(pMatch,2)
            fig_name = [res_dir,'Jaligned_',num2str(i),'_',num2str(j),'.jpg'];
            tformArr(i,j)={getTform(jMatch(i,j),views([i,j]),fig_name)};
            fprintf('%d-%d\n',i,j);
